...
describe('Testing fetch content (resolver)', () => {
  let action;
  let context;
  let logger;
  ...
  it('uses resolver for content proxy url', async () => {
    process.env.__OW_NAMESPACE = 'my-namespace';
    nock('https://raw.githubusercontent.com')
      .get('/adobe/test-repo/master/helix-markup.yaml')
      .reply(() => [404, 'Not Found']);
    ...
    action.resolver = {
      createURL() {
        return new URL('https://adobeioruntime.net/api/v1/web/my-namespace/helix-services/content-proxy@v2.2.3');
      },
    };
