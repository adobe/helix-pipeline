...
const doc1 = fs.readFileSync(path.resolve(__testdir, 'fixtures/example.md')).toString();
const data1 = 'none';
const html1 = fs.readFileSync(path.resolve(__testdir, 'fixtures/example.html')).toString();

const doc2 = fs.readFileSync(path.resolve(__testdir, 'fixtures/example-embeds.md')).toString();
const data2 = [{ foo: 'bar' }];
const html2 = fs.readFileSync(path.resolve(__testdir, 'fixtures/example-embeds.html')).toString();
...
{{post}}`,
  ...
  <h1 id="honda-fr-v">Honda FR-V</h1>
...
1. My car: [![{{make}} {{model}}]({{image}})](cars-{{year}}.md)`,
  ...
  it('test various embed format', async () => {
    const doc = fs.readFileSync(path.resolve(__testdir, 'fixtures/embeds.md')).toString();
    const data = 'none';
    const html = fs.readFileSync(path.resolve(__testdir, 'fixtures/embeds.html')).toString();
