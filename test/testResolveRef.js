...
const RESOLVE_GITREF_SERVICE =
  "https://adobeioruntime.net/api/v1/web/helix/helix-services/resolve-git-ref%40v1";
...
describe("Test resolve git ref", () => {
  ...
  it("Resolve ignores if no REPO_RAW_ROOT.", async () => {
    ...
    assert.ok(
      out.indexOf("cannot resolve ref. No REPO_RAW_ROOT specified.") > 0
    );
  });
  ...
  it("Resolve ignores if non github REPO_RAW_ROOT.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "unable to resolve ref to non-github repository: https://bitbucket.org/"
      ) > 0
    );
  });
  ...
  it("Resolve ignores if non github REPO_RAW_ROOT but ignores error for localhost.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "unable to resolve ref to non-github repository: https://localhost:1234/"
      ) < 0
    );
  ...
  it("Resolve ignores if non github REPO_RAW_ROOT but ignores error for 127.0.0.1.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "unable to resolve ref to non-github repository: https://127.0.0.1:1234/"
      ) < 0
    );
    assert.equal(action.request.params.ref, undefined);
  });
  ...
  it("Resolve ignores if RESOLVE_GITREF_SERVICE is missing.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "ignoring github ref resolving. RESOLVE_GITREF_SERVICE is not set."
      ) > 0
    );
  });
  ...
  it("Resolve resolves master ref correctly.", async () => {
    ...
    await resolveRef({}, action);
    assert.equal(
      action.request.params.ref,
      "565cb704628a094edde7e7f87d662b89559adcf0"
    );
  });
  ...
  it("Resolve resolves missing ref as master correctly.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "Recoverable error: no ref given for helix-cli/adobe, falling back to master"
      ) > 0
    );
    assert.equal(
      action.request.params.ref,
  ...
  it("Resolve ignores ref that looks like a SHA.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "github-ref '9932fb0fa5853920d204d368af41c6efb3f7486c' looks like a SHA. Ignoring resolving."
      ) > 0
    );
    assert.equal(
      action.request.params.ref,
      "9932fb0fa5853920d204d368af41c6efb3f7486c"
    );
  });
  ...
  it("Handles action error response.", async function test() {
    ...
    assert.ok(
      out.indexOf(
        'unable to resolve github-ref of master: 500 - "Internal Server Error"'
      ) > 0
    );
  ...
  it("Refues to invoke openwhisk outside environment.", async () => {
    ...
    assert.ok(
      out.indexOf(
        "unable to resolve github-ref of master: cannot invoke action outside openwhisk environment."
      ) > 0
    );
    assert.equal(action.request.params.ref, "master");
  });
  ...
  it("Handles action status error response.", async function test() {
    ...
    const out = await logger.getOutput();
    assert.ok(
      out.indexOf("unable to resolve github-ref of master: unknown error") > 0
    );
    assert.equal(action.request.params.ref, "master");
  });
});
