...
import { runPipeline } from '../src/universal.js';
import { pipe, setupPolly, universalRequest } from './utils.js';
...
describe('Testing HTML Pipeline (universal)', () => {
  setupPolly({
...
${context.content.document.body.innerHTML}`,
  ...
  it('html.pipe renders index.md from helix-cli correctly', async () => {
    const result = await pipe(
      ...
      },
      {
        request: crequest,
        content: {
          body: fs.readFileSync(path.resolve(__testdir, 'fixtures/index-unmodified.md')).toString(),
        },
      },
      {
        request: {
  ...
  it('html.pipe renders index.md from project-helix.io correctly', async () => {
    const result = await pipe(
      ...
      },
      {
        request: crequest,
        content: {
          body: fs.readFileSync(path.resolve(__testdir, 'fixtures/index-projecthelixio.md')).toString(),
        },
      },
      {
        request: {
  ...
  it('html.pipe renders modified index.md from helix-cli correctly', async () => {
    const result = await pipe(
      ...
      },
      {
        request: crequest,
        content: {
          body: fs.readFileSync(path.resolve(__testdir, 'fixtures/index-modified.md')).toString(),
        },
      },
      {
        request: {
  ...
  it('html.pipe via pipeline fetch errors are propagated to action response', async () => {
    ...
    assert.equal(out.errorMessage, 'Error while fetching file from https://raw.githubusercontent.com/adobe/helix-pipeline/master/not-existent.md: 404');
  });
