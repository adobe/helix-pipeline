...
async function fetch(context, { secrets = {}, request, logger }) {
  ...
  const { owner, repo, path, branch } = request.params;
  ...
  if (!ref) {
    logger.warn(
      `Recoverable error: no ref given for ${repo}/${owner}.git${path}, falling back to master`
    );
    ref = "master";
  }
  ...
  try {
    ...
    if (branch && branch !== ref) {
      setdefault(content, "sources", []).push(
        uri(rootPath, owner, repo, branch, path)
      );
    } else {
    ...
    if (e.statusCode === 404) {
      ...
      (e.response &&
        e.response.elapsedTime &&
        e.response.elapsedTime > timeout) ||
      (e.cause &&
        e.cause.code &&
        (e.cause.code === "ESOCKETTIMEDOUT" || e.cause.code === "ETIMEDOUT"))
    ) {
      // return gateway timeout
      logger.error(
        `Gateway timeout of ${timeout} milliseconds exceeded for ${options.uri}`
      );
      setdefault(context, "response", {}).status = 504;
    } else {
      logger.error(
        `Error while fetching Markdown from ${options.uri} with the following ` +
