...
function internalGatsbyEmbed(text, base, contentext, resourceext) {
  const matches = new RegExp(
    `^(markdown|html|embed): ?(.*(${contentext}|${resourceext}))$`
  ).exec(text);
...
function internalIaEmbed({ type, children }, base, contentext, resourceext) {
  if (
    ...
    !children[0].value.match(/ /) &&
    (children[0].value.endsWith(contentext) ||
      children[0].value.endsWith(resourceext))
  ) {
...
function internalImgEmbed({ type, children }, base, contentext, resourceext) {
  if (
    ...
    (children[0].url.endsWith(contentext) ||
      children[0].url.endsWith(resourceext))
  ) {
...
function embed(uri, node, whitelist = "", debug = () => {}) {
  if (
    ...
    mm.some(uri.host, whitelist.split(","))
  ) {
    const children = [{ ...node }];
    node.type = "embed";
...
function internalembed(uri, node, extension) {
  ...
  node.url = p.resolve(
    p.dirname(uri.path),
    p.basename(uri.path, p.extname(uri.path)) + extension
  );
  if (node.value) {
    delete node.value;
  }
...
function find(
  ...
  {
    logger,
    secrets: { EMBED_WHITELIST, EMBED_SELECTOR },
    request: {
      params: { path }
    }
  }
