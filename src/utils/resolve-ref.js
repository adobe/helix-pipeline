...
async function resolveRef(context, { secrets = {}, request, logger }) {
  ...
  if (
    rootURI.host !== "raw.github.com" &&
    rootURI.host !== "raw.githubusercontent.com"
  ) {
    if (rootURI.host !== "localhost" && rootURI.host !== "127.0.0.1") {
      logger.warn(
        `unable to resolve ref to non-github repository: ${secrets.REPO_RAW_ROOT}`
      );
    }
    return;
  }
  ...
  if (!ref) {
    logger.warn(
      `Recoverable error: no ref given for ${repo}/${owner}, falling back to master`
    );
    ref = "master";
  }
